...
um \textbf{cenário com vegetação e profundidade}, \textbf{partículas},
\textbf{áudio} e \textbf{dificuldade progressiva}. Os objetivos do trabalho
...
\begin{itemize}
  ...
  \item \textbf{miniaudio} --- reprodução dos efeitos sonoros.
  \item \textbf{Blender} --- modelagem e exportação de \emph{assets} 3D
        (por exemplo, o cano com borda separada, em formato \texttt{.glb}).
\end{itemize}

Os modelos da capivara e demais objetos foram obtidos em repositórios
livres (por exemplo, \emph{poly.pizza}) e, quando necessário, refeitos no
Blender para se adequarem ao jogo.
...
A cena utiliza uma fonte de luz pontual (\texttt{GL\_LIGHT0}). O
sombreamento é suave (\emph{Gouraud}), com normais por vértice calculadas no
carregamento dos modelos. O recurso \texttt{GL\_COLOR\_MATERIAL} permite
definir o material dos objetos diretamente pela cor, e \texttt{GL\_NORMALIZE}
mantém a iluminação correta mesmo quando os modelos são escalados.

\subsection{Texturas}
A capivara é texturizada a partir de uma imagem PNG (\emph{base color})
carregada com \emph{stb\_image} e mapeada pelas coordenadas de textura do
modelo. Modelos em \texttt{.glb}/\texttt{.obj} podem ainda ser coloridos
pelas cores definidas em seus materiais.
...
O jogo inclui ainda: um \textbf{sistema de partículas} (poeira, brilho e
explosão) desenhado com \emph{billboards} e \emph{blending}; um \textbf{HUD}
com placar e mensagens renderizado por \emph{stb\_truetype}; uma
\textbf{máquina de estados do jogo} (início, jogando, fim de jogo); e
\textbf{áudio} via \emph{miniaudio}.
...
A combinação de iluminação, sombreamento suave e textura confere volume aos
objetos, enquanto o \emph{z-buffer} e o \emph{back-face culling} garantem a
visibilidade correta. A colisão por AABB mostrou-se simples e eficaz para a
jogabilidade. A IA de piloto automático da abelha aliada demonstra o
requisito de inteligência artificial de forma marcante, pois ela
efetivamente \emph{joga} o jogo, navegando as brechas à frente do jogador.
